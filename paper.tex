...
measured flux density at a rest-frame wavelength between 8 and 1000$\,\micron$,
...
\begin{figure}[!ht]
\centering
\includegraphics[scale=0.32]{lir_luv_sample.eps}
...
fraction at rest-frame 0.66$\,\micron$.  For the
...
\citealt{overzier11}, \alephha~= \alephuv$^{XXXX}$.  We lack
...
galaxies with H$\alpha$-detected SFRs lies between \alephha~= \alephuv$^{XXXX}$
and \alephha~= \alephuv$^{XXXX}$.  A survival regression, which incorporates
H$\alpha$ SFR limits in a fit to the form $\log\,$\alephha~=
$q\,\log\,$\alephuv, finds $q = XXXX$, as does a simple linear regression using
...
\startdata
Low-z dust-poor & 0.766 & XXXX & XXXX & XXXX \\
Low-z dusty     & 0.532 & XXXX & XXXX & XXXX \\
High-z dusty    & 0.537 & XXXX & XXXX  & XXXX
\enddata
\end{deluxetable}
...
\citet{madau14} use the models of \citet{conroy10} to calculate
...
[\mathrm{duration}, f_{\mathrm{esc}}(0.16\,\micron)]$.  We set aside the
...
conversion factors differ from our adopted value in \S\ref{sfrs}.
\end{enumerate}
...
We perform a second experiment to investigate the effect of episodes of constant
...
uniform distribution with limits at 20 Myr and 15 Gyr.  We calculate the
...
sample of galaxies, record $q$, and repeat the process 10,000 times.  Fig.
\ref{} shows the distribution of $q$ values from this process.  While the
$q$ values bounding the central 50\% of the sample encompass the range of $q$
values in Fig. \ref{}, we caution that this simulation involves many
assumptions.
